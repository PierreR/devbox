...
====
- Please remove `cicd-shell` from `envPackages` in the `config/box.dhall` file.
+
Check https://github.com/CIRB/devbox/blob/master/user/config/box.dhall#L18[config/box.dhall] if you are unsure.
- Remove the `eclipse` entry from your local-configuration.nix
====
- Eclipse is now completely defined in user space
- Use https://cachix.org[cachix] to deploy the cicd-shell & puppet unit tests framework via a nix binary cache
