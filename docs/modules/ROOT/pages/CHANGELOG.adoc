...
A default bigger box is now generated from packer.
Re-packaging will provision the system (with the specific local-configuration) and the user.
...
[CAUTION,caption=Breaking changes]
====
- In the `config/box.dhall` file, please:
* rename `eclipsePlugins` with `eclipse`
* remove `cicd-shell` from `envPackages` in the `config/box.dhall` file.
+
Check https://github.com/CIRB/devbox/blob/master/user/config/box.dhall[config/box.dhall] if you are unsure.

- Remove the `eclipse` entry from your local-configuration.nix
====
- Eclipse is now completely defined in user space
- Creating a box is faster thanks to the use of https://cachix.org[cachix] binary cache
- `cicd-shell` updated to `2.4.2`
- `language-puppet` updated to `1.3.20`
- [Bug fix] Only install Taffybar when using the default desktop config

## v4.1, 28/05/2018
- `cicd-shell` updated to `2.3.1`
- Remove the `OS upgrade` step from `vagrant provision`.
- [Bug fix] Only install Albert when using the default desktop config

## v4.0, 11/05/2018

[CAUTION,caption=Breaking changes]
====
- Both `config/shell.dhall` and `config/box.dhall` needs to be modified according to the provided templates:
* https://github.com/CIRB/devbox/blob/master/user/config/shell.dhall[config/shell.dhall]
* https://github.com/CIRB/devbox/blob/master/user/config/box.dhall#L18[config/box.dhall]
...
- Add `desktop-gnome-configuration.nix`
- `language-puppet` updated to `1.3.17`
- `cicd-shell` updated to `2.0.9`
- New `sshi` shortcut to quicky connect to remote machines
