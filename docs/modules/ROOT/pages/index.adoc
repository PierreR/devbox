...
- Virtualbox 6.0.x (tested with 6.0.8)
...
[upperroman]
. Import the `ovf` image file using the Virtualbox UI.
...
====
The customization of the devbox operates at different levels and makes sure every configuration - personal or shared - survives the destruction of the box.
====
...
To avoid losing your customization, copy `/etc/nixos/local-configuration.nix` into the ROOT_DIR folder. When such a file exists, it is used at the first boot and is synchronized at each provisioning.
...
After the change, rebuild nixos using `updateSystem`.
...
This imperative approach is quite useful for testing but the change is not persistent. When you settle down on a list of packages, please add them to `local-configuration.nix` to gain persistence.
...
. Import the `ovf` image file using the Virtualbox UI.
. Add your shared directory folder using the Virtualbox UI and change the vm configuration if necessary.
