...
Within the host os (usually `Windows`), create a folder `C:\Users\virtualbox\devbox-x.x` where 'x.x' is the major and minor version number of the devbox.
You need to make sure your windows user has read/write permission on the folder (menu:Properties[Sharing]).

This will be your shared folder between the host (Windows) and the guest (the linux box).
...
You don't need to create any of the user configuration files. If a file is not yet present in the SHARED_DIR, it will be created at the first boot.
You should then edit the file and fill the requested informationfootnote:[a comment section is present at the top.].

[WARNING]
====
- If you don't use this default location, you will need to manually change the setting of the shared folder in Virtualbox. 
...
Get the latest `devbox-x.x.x` file from https://repository.irisnet.be/artifactory/webapp/#/artifacts/browse/tree/General/cicd-devbox[Artifactory]
