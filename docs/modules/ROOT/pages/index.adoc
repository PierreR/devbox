...
The box (of about 2G) is generated with packer (currently with packer-1.3.3):

. On the host (Windows), clone the https://github.com/nix-community/nixbox[nixbox repository]
. Copy `custom-configuration.nix`, `local-configuration.nix` and the default desktop config file to the `scripts` folder.
. Tweak `scripts/install.sh` to include these two lines:
+
```
...
+
```
packer.exe build nixos-x86_64.json --only virtualbox-iso <1>
vagrant box add devbox-x.x-pre packer_virtualbox-iso_virtualbox.box
```
<1> choose the hypervisor that match the value of `custom-configuration.nix`.

. Modify your vagrantfile to use the new registered box.
. Copy a default (tailored) `local-configuration.nix`
