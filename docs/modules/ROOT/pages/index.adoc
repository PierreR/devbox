...
[WARNING]
====
Some `mr` description files purposefully prevent the repository update by using the #skip# marker.
For these repositories, #updateConfig# won't do anything (stdout output: `skipped`).
In that case and more particularly when you first boot a new devbox, you might want to force the update.

In the vagrant home user, use the following command:

.force all mr repositories update
```
→ mr -f up <1>
```
