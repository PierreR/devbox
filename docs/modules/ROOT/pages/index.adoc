...
Customizations are possible on two levels:

- *system* through changes on your `local-configuration.nix`
- *user* through changes on your `config/box.dhall` configuration file.

As a user of the devbox, you can easily layer customizations tailored for your organization, your team or your personal interest.
...
- Virtualbox 5.2.x (tested with 5.2.26)
...
. In `ROOT_DIR`, copy the system configuration of your organization:
+
```
curl -o local-configuration.nix https://raw.githubusercontent.com/CIRB/devbox/master/system/cirb/local-configuration.nix
```
...
The provisioning is done automatically the first time `vagrant up` is executed.
...
Provisioning also brings the new features listed in the <<_changelog>>.
As long as no major devbox version is released, it is a safe operation that can be executed on a regular basis.
On the other hand, if a new devbox major version has been released, it is best to create a new devbox as described in <<_life_cycle>>.

You can also directly synchronize your customization changes on the guest box with:
...
This is always a safe operation because it takes its souce from a clone to the devbox in the `$HOME/bootstrap` directory.
...
NOTE: Alike `local-configuration.nix`, any change in those files can survive a `vagrant destroy` if you copy the file to your ROOT_DIR.
...
====
[lowerroman]
. add the new box to vagrant: `vagrant box add devbox-{latest_version} http://repo.irisnet.be/boxes/devbox.box`
. initialize the new box: `vagrant init devbox-{latest_version} && vagrant up`
. (after a while using the new version) check if you need to save anything from the previous box
. destroy the old box (all data will be lost)footnote:[The README describes several tricks to keep/store your customization.] : `vagrant destroy`
...
## Without vagrant or virtualbox

It is possible to use the devbox without the use of Vagrant or Virtualbox for instance within a Nixos linux or a vmware nixos guest.
...
. Create a ROOT_DIR
. With `curl`, retrieve from this repository:
+
- Vagrantfile file
- config/box.dhall & config/shell.dhall
- the local-configuration.nix file for your organization.
. Modify `Vagrantfile` to use the new registered box.
. Execute `vagrant up` and wait for the provisioning to complete.
. Reload and check the box: `vagrant reload`
. `vagrant package --output devbox-x.x.x.box --vagrantfile Vagrantfile`
