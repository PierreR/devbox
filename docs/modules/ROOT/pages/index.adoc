...
2. In `ROOT_DIR`, create a new config folder and copy the two files from the https://github.com/PierreR/devbox/tree/master/config[config folder]. Modify them to match your personal settings:
+
```
mkdir config
...
. `sudo make system config_file=/vagrant/shared/config/box.dhall`
. `make user config_file=/vagrant/shared/config/box.dhall`
====
