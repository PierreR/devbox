...
- `updateSystem`:
+
Update and (re)configure the system synchronizing 'ROOT_DIR/custom-configuration.nix' if present.
...
The general advice is to avoid system configuration. You should be able to configuration the box through the user configuration only.
If you really want to adjust the system, you can copy (or `curl`) https://github.com/CIRB/devbox/blob/master/system/configuration.nix[custom-configuration.nix] into your ROOTDIR and add your configuration into it.
