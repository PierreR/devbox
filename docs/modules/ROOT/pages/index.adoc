...
You don't need to create any of the user or system configuration files. If a file is not yet present in the SHARED_DIR, it will create there at the first boot.
You would then edit the template to fill in the requested informationfootnote:[a comment section is present at the top.].
...
The most common operation is to modify your user configuration through the `box.dhall` of your SHARED_DIR.
...
You can activate any available repositories by setting `repos` in 'SHARED_DIR/config/box'.
...
The devbox allows to quickly share any configuration across users. This is done by first adding a new `mr` source repository to the list of available repos http://stash.cirb.lan/projects/DEVB/repos/dotfiles/browse/.config/mr/available.d[in the mr CIRB template repository].
...
As soon as the PR is merged, any devbox user will be able to activate the new config by adding it to the list of its https://github.com/CIRB/devbox/blob/da7d4a1c38c0e0bd57806a486f3f42feb0492768/user/config/box.dhall#L18[`repos`] in `SHARED_DIR/config/box.dhall`.
...
You can register any source repository by adding it to https://github.com/CIRB/devbox/blob/4da9cf0a073e98dcdafab5100c156dc537adf57a/config/box-cirb.dhall#L45[mr.config] in `SHARED_DIR/config/box.dhall`. This is for personal configuration that you don't want to share. After saving the file, you update your box with #updateUser#.

TIP: You can use #mr.config# to extend the CIRB dotfiles configuration with your files. The top help section of https://github.com/CIRB/devbox/blob/4da9cf0a073e98dcdafab5100c156dc537adf57a/config/box-cirb.dhall#L17[SHARED_DIR/config/box-cirb.dhall] points to such a workflow as an example.


### Storing the box configuration file

You might want to go one step further and store your `box.dhall` file on Github or the like. This is pretty easy thanks to the powerful https://github.com/dhall-lang/dhall-lang/blob/master/README.md[dhall configuration language]. Replace the content of `SHARED_DIR/config/box.dhall` with one simple line containing an url that points to the config file. Here is an example:
