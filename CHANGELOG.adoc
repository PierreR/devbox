...
- The key `additionalRepos` deprecates the previous `mr_template_repo_url` field.
- The https://github.com/CIRB/vcsh_mr_template[mr template repository] is now generic and shouldn't be forked.

Press `F1` for more detailed instructions.

[NOTE]
We are using https://github.com/Gabriel439/Haskell-Dhall-Library[`dhall`], a statically typed configuration format. You will be warned beforehand if a configuration file is invalid.
...
The nixos window manager configuration has been isolated in a new file called `windowManager-configuration.nix`. Alike `local-configuratin.nix`, this file can be saved in your ROOT_DIR in case you want to use/try another window manager.

### New features

- https://albertlauncher.github.io/docs/[`albert`] (application launcer) on kbd:[<Control><Space>] replaces kbd:[<Super> p] (open menu)
- `nautilus` (file manager) enabled by default (see `local-configuration.nix`)
- `eclipse` updated to `neon` (4.6)
- `yedit` and `jdt` (eclipse plugin) enabled by default (see `local-configuration.nix`)
