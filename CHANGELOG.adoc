...
The `params.sh` file has been replaced by two files:
    ...
    - `config/box` (devbox data)
    - `config/shell` (cicd-shell data)
...
- The key `mrRepos` replaces the previous `user_stack` field and accepts a list of available `mr` repositories.
- The key `dotfilesGitRepo` deprecates the previous `mr_template_repo_url` field.
====
...
- `yedit` (eclipse plugin) enabled by default (see `local-configuration.nix`)
