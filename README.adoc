...
```
+
WARNING: If you create the file manually, make sure you keep an `unix` file encoding.
...
You can list all the repositories you wish to enable by setting `mrRepos` in 'ROOT_DIR/config/box'. The list of available repositories is defined https://github.com/CIRB/vcsh_mr_template/tree/master/.config/mr/available.d[here] .

All enabled repos will be automatically cloned and configured into your box. For instance if `puppet-bos` is in the list of `mrRepos`, you will find a `~/projects/cicd/puppet/bos` folder. In that case, the next step would be to go in that folder and to enter `make` on the command line.
...
Given the personal settings defined in the https://github.com/CIRB/devbox/blob/master/user/config/box[user/config/box] file, `vagrant provision --provision-with user` will:
...
You can add any `dotfiles` repositories including your own personal configuration thanks to a little tool called  https://github.com/RichiH/vcsh[vcsh].
...
As a note, if you want to override the CIRB dotfiles completely you can replace https://github.com/PierreR/vcsh_mr_template/commit/82708255d904beffe53b9587e8f553aa8804cc37[the pointer to the dotfiles]. In order to keep such a setting after a `vagrant destroy`, you need to update the `mrRepoUrl` value in `config/box`.
...
It is quite a useful approach for testing but such changes won't survive a `vagrant destroy`. When you settle down on a list of packages, please add them to `local-configuration.nix` to gain persistence.
