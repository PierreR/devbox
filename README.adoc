...
1. Within the host os (usually `Windows`), create a new folder of your choice. You might call it `devbox` or `devbox-x.x` where `x.x` is the major and minor version number of the devbox.
+
====
We will use the term `ROOT_DIR` to refer to this folder throughout the rest of the guide.
====
2. In `ROOT_DIR`, copy the https://github.com/CIRB/devbox/blob/master/user/params.sh[params file] and modify it to match your personal settings:
+
```
curl -o params.sh https://raw.githubusercontent.com/CIRB/devbox/master/user/params.sh
```
+
WARNING: Prefer #curl# or #wget#. If you create the file manually, you might end up with the wrong file encoding. In such situation, change the encoding back to `unix` in the editor of your choice. Because the file is readfootnote:[The file is directly `source` in a bash file. In devbox v3 a more structured format will be used.] from within the `devbox`, we need to avoid unwanted characters such as `CR LF`.
...
WARNING: Don't start the devbox from the Virtualbox UI. Use the command line (`vagrant up`). Otherwise the shared folder (/vagrant) won't be properly mounted.
...
- `system` to update and (re)configure the system
- `user` to (re)configure the vagrant user.

Updating the system takes a while and even more so if it is not done regularly. On the other hand, `vagrant provision --provision-with user` is a quick task.
====
...
- *atomicity*: configuration changes (`nixos-rebuild`) can be rollback.
...
If you set a valuefootnote:[Multiple values will be supported in devbox v3] for `user_stack` in the `params.sh` file, the puppet `mr` file for your stack will be activated for you. That means that the puppet source for your stack will be automatically cloned into the puppet project directory (`~/projects/cicd/puppet/$stackname`).
...
====
The customization of the devbox operates at different level and makes sure every configuration - personal or shared - survives a `vagrant destroy`.
====
...
WARNING: More drastic changes such a changing the window manager is only possible via `/etc/nixos/configuration.nix`. There is currently no support to persist such changes after a `vagrant destroy`.

### user

#### provision

Given the personal settings defined in the https://github.com/CIRB/devbox/blob/master/user/params.sh[params.sh] file, `vagrant provision --provision-with user` will:

- setup your git config
- synchronize the `ssh-keys` folder of the ROOT_DIR
- download requested eclipse plugins
- installed declared `mr` repositories including all https://github.com/CIRB/devbox-dotfiles[dotfiles]
- setup/download the cicd shell
...
For instance you might easily add/share some `geany`, `atom` or `eclipse` configurations.

To share simple dotfiles configuration across all devbox users, for instance let say you want to share a `~/.tmux.conf` file for the devbox, or make a change to an existing configuration you would do a pull request to the https://github.com/CIRB/devbox-dotfiles[CIRB dotfiles repositry]. Upon acceptance, the file will be part of the default CIRB dotfiles repository and will be pushed on all devboxes at the next provisioning.

#### mr

If you want to share across all devbox users a non trivial, external or optional configuration, you can use or create a specific source repository for it and register the repository https://github.com/CIRB/vcsh_mr_template/tree/master/.config/mr/available.d[in the mr CIRB template repository]. This is also done via a pull request.
...
Eventually there is yet another way to change the local user configuration. You normally won't need that option but it is there for flexibility sake. Some of your settings are personal and there is no need for them to be shared inside the CIRB organization. In that case, 3 steps are required:
...
As a note, if you want to override the CIRB dotfiles completely you can replace https://github.com/PierreR/vcsh_mr_template/commit/82708255d904beffe53b9587e8f553aa8804cc37[the pointer to the dotfiles]. In order to keep such a setting after a `vagrant destroy`, you need to update the `mr_template_repo_url` value in `params.sh`.
