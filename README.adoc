...
- Virtualbox 5.1.x (tested with 5.1.18)
- Vagrant 1.9.x (tested with 1.9.1)
...
[subs=attributes+]
```
vagrant box add devbox-{latest_version} http://repo.irisnet.be/boxes/devbox.box <1>
```
...
| kbd:[<Super> q] |Close (kill) current window
...
You can register any source repository by adding it to https://github.com/CIRB/devbox/blob/master/user/config/box#L8[`additionalRepos`] in `ROOT_DIR/config/box`. This is for personal configuration that you don't want to share. After saving the file, you update your box with `vagrant provision --provision-with user`.

TIP: You can use `additionalRepos` to extend the CIRB dotfiles configuration with your files. The top help section of `ROOT_DIR/config/box` points to such a workflow as an example.

WARNING: `additionalRepos` is meant as a static setting. It is only taken into account the first time you create a box. This is unlike `repos` that refreshes activated repositories each time you provision.
