...
WARNING: More drastic changes might conflict with the base configuration. If you wish to change the window manager you should change the `/etc/nixos/windowManager-configuration.nix` file.
...
You can register any source repository by adding it to the `additionalRepos` in `ROOT_DIR/config/box`. This is for personal configuration that you don't want to share. After saving the file, you update your box with `vagrant provision --provision-with user`.
