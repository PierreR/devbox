...
You can activate any available repository by setting `mrRepos` in 'ROOT_DIR/config/box'. The list of available repositories is defined https://github.com/CIRB/vcsh_mr_template/tree/master/.config/mr/available.d[here] .
...
`vcsh` allows to have multiple git repository installed in a single $HOME folder.
...
To activate the new repo, you just add it to the list of `mrRepos` in 'ROOT_DIR/config/box' and run `mr up` in the box.

#### forking the dotfiles repository

Eventually there is yet another way to change the local user configuration but in a more drastic way. You normally won't need that option but it is there for flexibility sake. You can use your own fork of the https://github.com/CIRB/devbox-dotfiles[standard dotfiles repository] by following these 4 steps:

1. vagrant destroy
2. Fork the https://github.com/CIRB/devbox-dotfiles[dotfiles repo]
3. Change the box configuration file to use the new repo (see `dotfilesGitRepo`)
4. vagrant up

WARNING: The burden of back-porting useful changes from the standard dotfiles repo will be on you.
