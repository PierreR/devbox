...
+
====
We will use the term `ROOT_DIR` to refer to this folder in the following sections.
====
2. In `ROOT_DIR`, copy the https://github.com/CIRB/devbox/blob/master/user/params.sh[params file] to automate your personal setup (git repos, git config, credentials, ...):
+
```
curl -o params.sh https://raw.githubusercontent.com/CIRB/devbox/master/user/params.sh
```
+
WARNING: To preserve the original encoding of the file (`unix` and not `Windows`) do not copy/paste from the Github UI. If you decide to manually create the param.sh file in the ROOT_DIR,  please verify and eventually change the file encoding using the editor of your choice. This file is read from within the `devbox` and  we want avoid unwanted characters such as `CR LF`.
