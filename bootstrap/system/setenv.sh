...
if [[ -d "$shared_dir"  ]] ; then
    /usr/bin/env time -f "Completed after %E min" nixos-rebuild switch --upgrade > "${shared_dir}/system_boot.log" 2>&1
else
    /usr/bin/env time -f "Completed after %E min" nixos-rebuild switch --upgrade
fi
