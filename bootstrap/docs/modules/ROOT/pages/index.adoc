...
The devbox provided an easy way to get started. Nevertheless if you prefer to work with another linux derivation (such as Ubuntu for instance) it is possible to do so and still benefit from the http://stash.cirb.lan/projects/CICD/repos/nixpkgs-overlays/browse[binary cache] the CICD team maintained.
