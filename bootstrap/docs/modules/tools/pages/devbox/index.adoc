...
====
- Install Virtualbox 6.1.x (tested with 6.1.14)
...
The `devbox` currently uses the https://github.com/thestinger/termite#keybindings[`termite`,role=external] terminal by default.
https://github.com/alacritty/alacritty[Alacritty, role=external] can also be usedfootnote:[Alacritty will likely be the default terminal in 21.03].
...
[subs=attributes+]
```
export NIX_PATH=$NIX_PATH:nixpkgs-overlays=http://stash.cirb.lan/CICD/nixpkgs-overlays/archive/{latest_version}.tar.gz
```
. Add the current nixpkgs pointer
+
```
cat > ~/.nix-channels <<EOF
https://releases.nixos.org/nixos/20.09/nixos-20.09.1870.14cf906f33c/nixexprs.tar.xz nixpkgs
EOF
```
IMPORTANT: You will need to manually follow the update of http://stash.cirb.lan/projects/CICD/repos/puppet-profile/browse/manifests/nix.pp#27[the pointer].
...
. On  a *linux* hostfootnote:[cloning in Windows will cause many encoding issues!], use the provided `nixbox.sh` script to clone the https://github.com/nix-community/nixbox[nixbox repository,role=external] within the shared directory.
