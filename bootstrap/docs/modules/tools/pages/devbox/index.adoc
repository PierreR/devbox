...
The `devbox` currently uses the https://github.com/thestinger/termite#keybindings[`termite`] terminal by default.
Alacritty can also be usedfootnote:[Alacritty will likely be the default terminal in 21.03].
...
You would commonly need to make some adjustments to your personal configuration file such as `dhall.box` or `local-configuration.nix` before importing them in the new SHARED_DIR.
...
https://releases.nixos.org/nixos/20.09/nixos-20.09.1870.14cf906f33c/nixexprs.tar.xz nixpkgs <1>
EOF
```
<1> You will need to manually follow the update of http://stash.cirb.lan/projects/CICD/repos/puppet-profile/browse/manifests/nix.pp#27[the pointer].
